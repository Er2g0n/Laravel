                ...
                <tbody>
                    @foreach ($users as $user)
                        <tr>
                            ...
                            <td>
                                <span
                                    class="right badge badge-{{ $user->status == 1 ? 'success' : 'dark' }}">{{ $user->status == 1 ? 'Show' : 'Hidden' }}
                                </span>
                            </td>
                            <td>{{ $user->full_name }}</td>
                            <td>{{ $user->phone }}</td>
                            <td>{{ date('d/m/y -H:m:i', strtotime($user->created_at)) }}</td> {{-- định dạng lại theo năm tháng ngày --}}
                            <td><a href="{{ route('admin.user.edit', ['id' => $user->id]) }}">Edit</a></td>
                            {{-- nhập theo route bên web --}}
                            <td><a onClick="return confirmDelete('user')"
                                    href="{{ route('admin.user.destroy', ['id' => $user->id]) }}">Delete</a>
